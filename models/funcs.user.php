...
function persNrExists($persNr)
{
	global $db,$db_table_prefix;

	$sql = "SELECT Active	FROM ".$db_table_prefix."Users WHERE
				Personalnr = '".$db->sql_escape($persNr)."'	LIMIT 1";
...
function fetchUserDetails($usermail=NULL,$token=NULL)
{
	global $db,$db_table_prefix;
	...
	if($usermail!=NULL)
	{
		$sql = "SELECT * FROM ".$db_table_prefix."Users
					WHERE
					Email = '".$db->sql_escape(sanitize($usermail))."'
					LIMIT
					1";
	}
	if($token != NULL)
...
function getGroupIdFromGroupName($groupName)
{
	...
	if($result)
	{
 	...
 	{
 		//no group with given name found
 		return -1;
 	}
}
?>